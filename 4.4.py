...
def Main():
    ...
    if proxy_list:
        ...
        try:
            # Tách đúng phần user/pass và host/port
            user_pass, host_port = current_proxy.split('@')
            ...
            # Gửi request kiểm tra proxy đến API proxymart
            response = requests.post('https://api.proxymart.net/api/check-proxy', json=data, timeout=10)
            check = response.json()
            ...
            if is_live:
                print(f'[✅ LIVE] Proxy hoạt động: {current_proxy} | IP: {myip}')
            else:
                print(f'[❌ DEAD] Proxy không hoạt động: {current_proxy}')
                myip = None
    
        except Exception as e:
            print(f'[❌ ERROR] Lỗi khi kiểm tra proxy: {current_proxy} → {e}')
    banner()
    while True:
